...
run_cpss <- function(data, q = 10, PFER = 2){
  require(mboost)
  require(parallel)
  ...
  mboost_fit <- glmboost(x = cbind(Intercept = 1, data$x), 
                          y = as.factor(data$y),
  ...
  stabs_rsmp <- subsample(model.weights(mboost_fit), B = 50, strata = mboost_fit$response)
  
  mboost_stabsel <- stabsel(mboost_fit, q = q, PFER = PFER, 
                               sampling.type = "SS",
                               assumption = "unimod",  
                               folds  = stabs_rsmp,
                               grid = 0:500)
  
  return(list(data = data, result = mboost_stabsel))
}
